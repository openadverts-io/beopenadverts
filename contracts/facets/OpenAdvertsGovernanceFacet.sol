 ...
 */
contract OpenAdvertsGovernanceFacet is ReentrancyGuard {
    ...
    // Per-round candidate roster, emitted as the round concludes so indexers can
    // reconstruct history despite proposedAdminAddresses being cleared afterwards.
    event AdminCandidatesSnapshot(uint256 indexed adminVoteId, address[] candidates);
     ...
     */
    function applyAsNewAdmin(string memory storageId, address newRequestKey) public payable {
        require(!LibOpenAdvertsPauseStorage.openAdvertsPauseStorage().paused, "System paused");
        LibOpenAdvertsGovernanceStorage.GovernanceStorage storage govStorage = LibOpenAdvertsGovernanceStorage.governanceStorage();
        LibOpenAdvertsTokenStorage.TokenStorage storage tokenStorage = LibOpenAdvertsTokenStorage.tokenStorage();
        ...
        // The candidate's requestKey becomes live only if they win; reject invalid keys up-front.
        // Extracted to OpenAdvertsRequestKeyFacet via self-call to keep this facet within the EIP-170 limit.
        IOpenAdvertsRequestKeyOps(address(this)).validateRequestKeyOnlyDiamond(newRequestKey, msg.sender);
     ...
     */
    function ratifyNewAdmin() public nonReentrant {
        LibOpenAdvertsGovernanceStorage.GovernanceStorage storage govStorage = LibOpenAdvertsGovernanceStorage.governanceStorage();
        LibOpenAdvertsTokenStorage.TokenStorage storage tokenStorage = LibOpenAdvertsTokenStorage.tokenStorage();
        LibDiamond.DiamondStorage storage diamondStorage = LibDiamond.diamondStorage();
        ...
        // Pay the outgoing admin's accrued USDC commission before the handover. Non-blocking:
        // TokenFacet escrows to pendingAdminUSDC (claimable via withdrawPendingAdminUSDC) if the
        // push fails, so a hostile incumbent cannot block their own unseating. Extracted to keep
        // this facet within the EIP-170 limit and to co-locate USDC accounting in TokenFacet.
        address outgoingAdmin = diamondStorage.contractOwner;
        if (selectedCandidate != outgoingAdmin) {
            IOpenAdvertsTokenFacet(address(this)).payoutOutgoingAdminCommission(outgoingAdmin);
            // Activate the winner's requestKey only on a real handover; an incumbent re-election
            // keeps the current key. Extracted to OpenAdvertsRequestKeyFacet via self-call for EIP-170 headroom.
            IOpenAdvertsRequestKeyOps(address(this)).rotateRequestKeyOnlyDiamond(
                govStorage.adminApplicantRequestKey[selectedCandidate][govStorage.adminVoteId],
                msg.sender
            );
        }
        ...
        // Emit the concluded round's candidate roster before clearing it (pre-increment id).
        emit AdminCandidatesSnapshot(govStorage.adminVoteId, govStorage.proposedAdminAddresses);
